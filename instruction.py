...
class Instruction:
    ...
    def obtain_salient_task_key(self, current_examples):
        ...
        This is necessary because Examples are generated randomly, so we need to infer the salient task that would have produced the Example"
        ...
        this means that the example which we will need to focus on to determine the salient task is Example 1

        because query.active_task_label = True,
        and query.task_a_label == salient_example.task_a_label,
        and query_task_a_label,
        we return ('task_a', True), meaning that the first task from the first example is the task determining the instruction

        let's see a Prompt for which this would occur (and the corresponding task labels remain the same as previously discussed):
    ...
    def set_salient_task(self, current_examples, include_ambiguous_examples, salient_task_a_or_b):
        '''
        ...
        All of the examples in a Prompt will have the same salient_task value and as such this value is set once the Prompt has already been made

        For example, in lengthy example provided in the docstring for obtain_salient_task_key(), all 3 of the Examples would have their salient_task variable 
        be set to 'subject' as that is the task that determined the instruction. This is needed when visualizing the results to provide each task with its own bar
        ...
        if include_ambiguous_examples:
            ...
            salient_task_key = self.create_salient_task_key(current_examples, salient_task_a_or_b)
        ...
        construction_type_map[self.construction_type].set_custom_salient_task(salient_task_key, current_examples)
    ...
    def make_instruction(self, current_examples, include_ambiguous_examples, salient_task_a_or_b):
        """
        ...
        instruction = construction_type_map[self.construction_type].make_instruction(salient_task_key)
        return instruction

    def set_custom_salient_task(self, salient_task_key, current_examples):
        if salient_task_key[0] == 'task_a':
            for e in current_examples:
                e.salient_task = self.tasks[0]
        else:
            for e in current_examples:
                e.salient_task = self.tasks[1]
